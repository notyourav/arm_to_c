...
ARGUMENT_REGS = list(map(Register, ["r0", "r1", "r2", "r3"]))
...
def get_stack_info(
    ...
    for inst in start_node.block.instructions:
        ...
        if inst.mnemonic == "jal":
            break
        elif inst.mnemonic == "add" and destination.register_name == "sp":
            ...
            info.is_leaf = False
            lr_loc = inst.args.index("lr")
            info.return_addr_location = inst.args[lr_loc].lhs_as_literal()

            # since there may be more to be pushed
            inst.args.pop(lr_loc)
            start_node.block.instructions.insert(instindex, inst)
        elif (
...
CASES_STORE: StoreInstrMap = {
    ...
    "strb": lambda a: make_store(a, type=Type.of_size(8)),
    "strh": lambda a: make_store(a, type=Type.of_size(16)),
    "str": lambda a: make_store(a, type=Type.of_size(32)),
    # Unaligned stores
...
CASES_BRANCHES: CmpInstrMap = {
    ...
    "beq": lambda a: BinaryOp.icmp(a.reg(0), "==", Literal(0)),
    "bne": lambda a: BinaryOp.icmp(a.reg(0), "!=", Literal(0)),
    "ble": lambda a: BinaryOp.scmp(a.reg(0), "<=", Literal(0)),
    "bgt": lambda a: BinaryOp.scmp(a.reg(0), ">", Literal(0)),
    "blt": lambda a: BinaryOp.scmp(a.reg(0), "<", Literal(0)),
    "bge": lambda a: BinaryOp.scmp(a.reg(0), ">=", Literal(0)),
}
...
CASES_HI_LO: PairInstrMap = {
    ...
    "mul": lambda a: (None, BinaryOp.int(a.reg(0), "*", a.reg(1))),
    "muls": lambda a: (None, BinaryOp.int(a.reg(0), "*", a.reg(1))),
...
CASES_DESTINATION_FIRST: InstrMap = {
    ...
    # Integer arithmetic
    "add": lambda a: handle_addi(a),
    "adds": lambda a: handle_addi(a),
    "addu": lambda a: handle_add(a),
    "subs": lambda a: fold_mul_chains(BinaryOp.intptr(a.reg(1), "-", a.reg(2))),
    "rsbs": lambda a: fold_mul_chains(
        UnaryOp(op="-", expr=as_s32(a.reg(1)), type=Type.s32())
    ),
    "rsb": lambda a: fold_mul_chains(
        UnaryOp(op="-", expr=as_s32(a.reg(1)), type=Type.s32())
    ),
    ...
    # Bit arithmetic
    "ands": lambda a: BinaryOp.int(left=a.reg(1), op="&", right=a.reg(2)),
    "orrs": lambda a: BinaryOp.int(left=a.reg(1), op="|", right=a.reg(2)),
    ...
    "eor": lambda a: BinaryOp.int(left=a.reg(1), op="^", right=a.reg(2)),
    ...
    "ldr": lambda a: handle_load(a, type=Type.intptr32()),
